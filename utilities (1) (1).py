...
def redness(img: Image.Image) -> float:
    ...
    R = M[:, :, 0] * 1.0
    G = M[:, :, 1] * 1.0
    # Moyenne sur les pixels de premier plan
...
def blueness(img: Image.Image) -> float:
    ...
    B = M[:, :, 2] * 1.0
    # Moyenne sur les pixels de premier plan
    return np.mean(B[F]) - np.mean(G[F])
...
def error_rate(solutions: pd.Series, predictions: pd.Series) -> Any:
    ...
    n = len(solutions)
    for i in range(n):
        if solutions.iloc[i] != predictions[i]:
            e += 1
    return e/n
...
class OneRule:
    ...
    def fit(self, X: pd.DataFrame, Y: pd.Series) -> None:
        ...
        m_pos = X.loc[Y == 1, self.attribute].mean()
        m_neg = X.loc[Y == -1, self.attribute].mean()
        self.theta = 0.5 * (m_pos + m_neg)
